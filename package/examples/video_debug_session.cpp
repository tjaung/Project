                         ...
                         std::size_t anchor_count)
{
    ...
    DrawTextBlock(annotated, current, previous, runner_state, anchor_count);
...
int main(int argc, char** argv)
{
    ...
    {
        std::cerr << std::endl
                  << "Usage: ./video_debug_session path_to_vocabulary path_to_settings video_source" << std::endl;
        return 1;
    }
    ...
    {
        std::cerr << "Failed to start video session." << std::endl;
        return 1;
    }

    cv::namedWindow("video_debug_session", cv::WINDOW_NORMAL);
    ...
    while(true)
    {
        ...
        const monocular_slam::RunnerState state = snapshot.runner_state;
        ...
        if(current.frame_id != last_displayed_frame_id && !current.frame_bgr.empty())
        {
            ...
            cv::Mat debug_view = ComposeDebugView(frame, current, previous, state, camera_positions, accumulated_scene_points, anchors.size());
            cv::imshow("video_debug_session", debug_view);
        ...
        if(key == 'a' || key == 'A')
        {
            monocular_slam::Anchor anchor = session.ar.CreateAnchorAtCurrentPose("live-anchor");
            if(anchor.valid)
