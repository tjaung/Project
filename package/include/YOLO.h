...
namespace ORB_SLAM3
{
    ...
    class YOLO
    {
        public:
            ...
            void InsertInput(const cv::Mat& imRGB, const cv::Mat& imDepth, int frameId = -1);
            std::vector<cv::Mat> GetOutput();
            std::vector<TelemetryDetection> GetLastDetections() const;
            int GetLastOutputFrameId() const;
            ...
            mutable std::mutex mDetectionMutex;
            mutable std::mutex mFrameIdMutex;
        ...
        private:
            ...
            std::vector<TelemetryDetection> mLastDetections;
            int mInputFrameId = -1;
            int mLastOutputFrameId = -1;
            cv::dnn::Net net;
