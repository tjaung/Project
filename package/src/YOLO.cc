...
namespace ORB_SLAM3
{
	...
	cv::Mat YOLO::postprocess(const cv::Mat& imRGB, const cv::Mat& imDepth, const std::vector<cv::Mat>& outs, const float maxBoxRatio)   // Remove the bounding boxes with low confidence using non-maxima suppression
	{
		...
		for (size_t i = 0; i < indices.size(); ++i)
		{
			...
			if(classIds[idx] >= 0 && classIds[idx] < static_cast<int>(classes.size()))
				detection.class_name = classes[classIds[idx]];
		...
		{
			std::unique_lock<std::mutex> lock(mDetectionMutex);
			mLastDetections = std::move(detections);
		}
	...
	void YOLO::InsertInput(const cv::Mat& imRGB, const cv::Mat& imDepth, int frameId)
	{
		std::unique_lock<std::mutex> lock(mInputMutex);
		mInputPair.clear();
		...
		{
			std::unique_lock<std::mutex> frameLock(mFrameIdMutex);
			mInputFrameId = frameId;
		}
	}
	...
	std::vector<TelemetryDetection> YOLO::GetLastDetections() const
	{
		std::unique_lock<std::mutex> lock(mDetectionMutex);
		return mLastDetections;
	}

	int YOLO::GetLastOutputFrameId() const
	{
		std::unique_lock<std::mutex> lock(mFrameIdMutex);
		return mLastOutputFrameId;
	}
	...
	void YOLO::Run()
	{
		...
		while(1)
		{
			...
			if(mInputPair.size() == 2)
			{
				...
				{
					std::unique_lock<std::mutex> frameLock(mFrameIdMutex);
					mLastOutputFrameId = inputFrameId;
				}
			}
			else
			{
