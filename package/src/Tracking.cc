...
TelemetryPoint2D ToTelemetryPoint(const cv::Point2f& point)
{
    TelemetryPoint2D out;
    out.x = point.x;
    out.y = point.y;
    return out;
}

TelemetryPoint3D ToTelemetryPoint(const cv::Point3f& point)
{
    TelemetryPoint3D out;
    out.x = point.x;
    out.y = point.y;
    out.z = point.z;
    return out;
...
size_t CountTrackedMapPoints(const std::vector<MapPoint*>& map_points)
{
    size_t count = 0;
...
void RefreshLastFrameLKCache(Frame& destination, const Frame& source)
{
    destination.mnId = source.mnId;
    destination.mTimeStamp = source.mTimeStamp;
    destination.mvKeysUn = source.mvKeysUn;
    destination.mvpMapPoints = source.mvpMapPoints;
    destination.N = source.N;

    if(source.HasPose())
        destination.SetPose(source.GetPose());
}
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename)
{
    ...
    else
    {
        ...
        const bool trackingHealthy = (mState == OK || mState == OK_KLT);
        if(trackingHealthy && mCurrentFrame.mpReferenceKF)
        {
    ...
    RefreshLastFrameLKCache(mLastFrameLK, mCurrentFrame);
    mFrameNum++;
...
{
    mPreviousFrameTelemetry = mCurrentFrameTelemetry;
    mCurrentFrameTelemetry = FrameTelemetry();
    mCurrentFrameTelemetry.timestamp = timestamp;
    mCurrentFrameTelemetry.tracking_state = mState;
    mCurrentFrameTelemetry.tracking_ok = (mState == OK || mState == OK_KLT);
    ...
    if(!(mpDepthAnythingV2 && mpYOLO))
    {
        ...
        mCurrentFrameTelemetry.frame_id = mCurrentFrame.mnId;
        mCurrentFrameTelemetry.pose = mCurrentFrame.GetPose();
        mCurrentFrameTelemetry.has_pose = mCurrentFrame.HasPose();
        mCurrentFrameTelemetry.tracking_state = mState;
        mCurrentFrameTelemetry.tracking_ok = (mState == OK || mState == OK_KLT);
        mCurrentFrameTelemetry.is_keyframe = mCurrentFrame.mbIsKeyFrame;
        mCurrentFrameTelemetry.raw_orb_keypoints = mCurrentFrame.mvKeysRaw;
        mCurrentFrameTelemetry.filtered_orb_keypoints = mCurrentFrame.mvKeysFiltered;
        mCurrentFrameTelemetry.orb_keypoints = mCurrentFrame.mvKeysUn;
        mCurrentFrameTelemetry.tracked_keypoints = mCurrentFrame.mvKeysUn;
        mCurrentFrameTelemetry.tracked_map_point_count = CountTrackedMapPoints(mCurrentFrame.mvpMapPoints);
        Map* pCurrentMap = (mpAtlas ? mpAtlas->GetCurrentMap() : static_cast<Map*>(NULL));
        if((mState == OK || mState == OK_KLT) && pCurrentMap)
        {
            mCurrentFrameTelemetry.active_map_keyframe_count = pCurrentMap->KeyFramesInMap();
            mCurrentFrameTelemetry.active_map_point_count = pCurrentMap->MapPointsInMap();
        }
        mCurrentFrameTelemetry.has_reference_keyframe = (mCurrentFrame.mpReferenceKF != nullptr);
    ...
    {
        mCurrentFrameTelemetry.depth_source_frame_id = completedDepthFrameId;
        completedDepth8u.convertTo(mImDepth2, CV_32F, 1.0 / 255.0);
        mCurrentFrameTelemetry.estimated_depth = mImDepth2.clone();
        ...
        if(completedDepthFrameId > mnLastDepthFrameSubmittedToYOLO)
        {
            for(size_t i = 0; i < mMonoDepthFrameCache.size(); ++i)
            {
                if(std::get<0>(mMonoDepthFrameCache[i]) == completedDepthFrameId)
                {
                    ...
                    mpYOLO->InsertInput(std::get<2>(mMonoDepthFrameCache[i]), refreshDepth, completedDepthFrameId);
    ...
    std::vector<cv::Mat> outputYOLO = mpYOLO->GetOutput();
    mCurrentFrameTelemetry.yolo_detections = mpYOLO->GetLastDetections();
    mCurrentFrameTelemetry.yolo_source_frame_id = mpYOLO->GetLastOutputFrameId();
    ...
    if(outputYOLO.size() == 2 && mFrameNum == 1)
    {
        ...
        mCurrentFrameTelemetry.yolo_output_consumed = true;
        mCurrentFrameTelemetry.mask_updated_from_yolo = updatedCurrentMaskEstimate;
        mCurrentFrameTelemetry.mask_source_frame_id = mCurrentFrameTelemetry.yolo_source_frame_id;
    }
    else if(outputYOLO.size() == 2 && mFrameNum > 1)
    {
        Eigen::Quaternionf q = mLastFrameLK.GetPose().unit_quaternion();
        ...
        if(!(maskSizeYOLO == 0 && maskSizeLast > 0 && isLargeRotation))
        {
            mImGrayLastKey = outputYOLO[0];
            mImMaskLastKey = outputYOLO[1];
            mCurrentFrameTelemetry.mask_updated_from_yolo = !outputYOLO[1].empty();
            mCurrentFrameTelemetry.mask_source_frame_id = mCurrentFrameTelemetry.yolo_source_frame_id;
        }
        mCurrentFrameTelemetry.yolo_output_consumed = true;
    }
    ...
    if(mFrameNum > 1 && !mImMaskLastKey.empty() && !mImDepth2.empty())
    {
        PredictCurrentMask();
        updatedCurrentMaskEstimate = !mImMask.empty();
        mCurrentFrameTelemetry.mask_updated_from_propagation = updatedCurrentMaskEstimate;
        if(updatedCurrentMaskEstimate)
            mCurrentFrameTelemetry.mask_source_frame_id = mFrameNum;
    }

    if(mbStartOpticalFlow)
    {
        mCurrentFrameTelemetry.used_optical_flow = true;
        mCurrentFrame = Frame(mbStartOpticalFlow);
        mCurrentFrame.mTimeStamp = timestamp;
    ...
    mCurrentFrameTelemetry.requested_new_keyframe = mbNeedKF;
    ...
    mCurrentFrameTelemetry.tracking_ok = (mState == OK || mState == OK_KLT);
    mCurrentFrameTelemetry.is_keyframe = mCurrentFrame.mbIsKeyFrame;
    mCurrentFrameTelemetry.dynamic_mask = mImMask.clone();
    mCurrentFrameTelemetry.estimated_depth = mImDepth2.clone();
    mCurrentFrameTelemetry.raw_orb_keypoints = mCurrentFrame.mvKeysRaw;
    mCurrentFrameTelemetry.filtered_orb_keypoints = mCurrentFrame.mvKeysFiltered;
    mCurrentFrameTelemetry.orb_keypoints = mCurrentFrame.mvKeysUn;
...
bool Tracking::TrackWithOpticalFlow()
{
    ...
    mCurrentFrameTelemetry.optical_flow_tracks.clear();
    ...
    {
        TelemetryOpticalFlowTrack track;
        if(i < static_cast<int>(mLastFrameLK.mvKeysUn.size()))
            track.previous_point = ToTelemetryPoint(mLastFrameLK.mvKeysUn[i].pt);
        track.current_point = ToTelemetryPoint(currKeyPoints[i]);
        track.status = true;
        track.inlier = (inliersSet.find(i) != inliersSet.end());
        mCurrentFrameTelemetry.optical_flow_tracks.push_back(track);
...
void Tracking::Reset(bool bLocMap)
{
    ...
    mCurrentFrameTelemetry = FrameTelemetry();
    mPreviousFrameTelemetry = FrameTelemetry();
...
cv::Mat Tracking::GetCurrentDynamicMask() const
{
    if(mImMask.empty())
        return cv::Mat();
    return mImMask.clone();
}

cv::Mat Tracking::GetCurrentEstimatedDepth() const
{
    if(mImDepth2.empty())
        return cv::Mat();
    return mImDepth2.clone();
}

Sophus::SE3f Tracking::GetCurrentPose() const
{
    return mCurrentFrame.GetPose();
}

FrameTelemetry Tracking::GetCurrentFrameTelemetry() const
{
    return mCurrentFrameTelemetry;
}

FrameTelemetry Tracking::GetPreviousFrameTelemetry() const
{
    return mPreviousFrameTelemetry;
}
...
void Tracking::PredictCurrentMask()
{
    ...
    mCurrentFrameTelemetry.mask_propagation = MaskPropagationTelemetry();
    mCurrentFrameTelemetry.mask_propagation.attempted = true;
    ...
    for(const cv::Point2f& point : lastDynaPoints)
        mCurrentFrameTelemetry.mask_propagation.seed_points.push_back(ToTelemetryPoint(point));
    ...
    if(!validLastDynaPoints.empty())
    {
        ...
        for(size_t j = 0; j < status.size(); ++j)
        {
            if(status[j]) 
            {
                ...
                mCurrentFrameTelemetry.mask_propagation.flowed_points.push_back(ToTelemetryPoint(currPt));
                ...
                if (depth >= 0.05)
                {
                    cv::Point3f trackedPoint(currDynaPoints[j].x, currDynaPoints[j].y, depth);
                    trackedDynaPoints.push_back(trackedPoint);
                    mCurrentFrameTelemetry.mask_propagation.tracked_points.push_back(ToTelemetryPoint(trackedPoint));
                }
            }
        }
        ...
        for(const auto& cluster : clusters)
        {
            ...
            telemetryCluster.id = cluster.first;
            for(const cv::Point3f& point : cluster.second)
                telemetryCluster.points.push_back(ToTelemetryPoint(point));
            mCurrentFrameTelemetry.mask_propagation.clusters.push_back(std::move(telemetryCluster));
        }
        CreateMaskFromClusters(clusters);
        mCurrentFrameTelemetry.mask_propagation.succeeded = !mImMask.empty() && cv::countNonZero(mImMask) > 0;
    }
}
