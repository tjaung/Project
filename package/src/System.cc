...
void System::SaveTrajectoryEuRoC(const string &filename)
{
    ...
    {
        std::cout << "No non-empty map available for trajectory export." << std::endl;
        return;
    }
    ...
    if(vpKFs.empty())
    {
        std::cout << "No keyframes available for trajectory export." << std::endl;
        return;
    }
    sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);
...
cv::Mat System::GetCurrentDynamicMask()
{
    unique_lock<mutex> lock(mMutexState);
    return mpTracker->GetCurrentDynamicMask();
}

cv::Mat System::GetCurrentEstimatedDepth()
{
    unique_lock<mutex> lock(mMutexState);
    return mpTracker->GetCurrentEstimatedDepth();
}

FrameTelemetry System::GetCurrentFrameTelemetry()
{
    unique_lock<mutex> lock(mMutexState);
    return mpTracker->GetCurrentFrameTelemetry();
}

FrameTelemetry System::GetPreviousFrameTelemetry()
{
    unique_lock<mutex> lock(mMutexState);
    return mpTracker->GetPreviousFrameTelemetry();
}

long unsigned int System::GetCurrentFrameId()
{
    unique_lock<mutex> lock(mMutexState);
