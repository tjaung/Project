...
struct AppState
{
    ...
    double lastX = 0.0;
    double lastY = 0.0;
    std::string loadedFrameImagePath;
    time_t loadedFrameMTime = 0;
};
...
bool LoadState(const std::string& path, ViewerState& state)
{
        ...
        return false;
    {
        ...
        if (values.size() != 6 && values.size() != 12)
            return false;
        ...
        {
            for (int i = 0; i < 9; ++i)
                state.cameraRotation.m[i] = values[3 + i];
        }
    }
    ...
    {
        state.frameImagePath = line.substr(6);
        if (!std::getline(in, line))
            return false;
    }
    else
    {
        state.frameImagePath.clear();
    }
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        pointLine >> point.position.x >> point.position.y >> point.position.z >> ref;
        if (!pointLine)
            return false;
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        std::istringstream keyframeLine(line);
        std::vector<float> values;
        keyframeLine >> keyframe.id;
        if (!keyframeLine)
            return false;

        float value = 0.0f;
        while (keyframeLine >> value)
            values.push_back(value);

        if (values.size() != 3 && values.size() != 12 && values.size() != 14)
            return false;

        keyframe.position = {values[0], values[1], values[2]};
        keyframe.rotation = Mat3{};
        if (values.size() >= 12)
        {
            for (int j = 0; j < 9; ++j)
                keyframe.rotation.m[j] = values[3 + j];
        }
        if (values.size() == 14)
        {
            keyframe.isRoot = values[12] > 0.5f;
            keyframe.mapId = static_cast<int>(values[13]);
        }
    ...
    for (size_t i = 0; i < count; ++i)
    {
        ...
        edgeLine >> edge.a.x >> edge.a.y >> edge.a.z
...
void DrawEdges(const AppState& app)
{
    glLineWidth(1.5f);
    glBegin(GL_LINES);
    for (const EdgeSample& edge : app.viewerState.edges)
    {
        if (edge.kind == 3)
        {
            if (!app.orbit.showInertial)
                continue;
            glColor3f(1.0f, 0.0f, 0.0f);
        }
        else if (!app.orbit.showEdges)
        {
            continue;
        }
        else if (edge.kind == 2)
...
void DrawFrustum(const Vec3& position, const Mat3& rotation, float scale, float red, float green, float blue, float lineWidth)
{
    const float w = scale;
...
void DrawKeyframes(const ViewerState& state)
{
    for (const KeyframeSample& keyframe : state.keyframes)
    {
        if (keyframe.isRoot)
            DrawFrustum(keyframe.position, keyframe.rotation, 0.11f, 1.0f, 0.0f, 0.0f, 2.5f);
        else
            DrawFrustum(keyframe.position, keyframe.rotation, 0.08f, 0.0f, 0.0f, 1.0f, 1.5f);
    }
}

void DrawCurrentCamera(const ViewerState& state)
{
    DrawFrustum(state.cameraPosition, state.cameraRotation, 0.14f, 0.0f, 1.0f, 0.0f, 3.0f);
}
...
void AutoFrameOrbit(AppState& app)
{
    ...
    app.orbit.distance = std::min(std::max(radius * 2.0f, 2.5f), 80.0f);
    app.orbit.yaw = 0.8f;
...
void MouseButtonCallback(GLFWwindow* window, int button, int action, int)
{
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT)
        app->leftDragging = action == GLFW_PRESS;
    if (button == GLFW_MOUSE_BUTTON_RIGHT)
        app->rightDragging = action == GLFW_PRESS;
...
void KeyCallback(GLFWwindow* window, int key, int, int action, int)
{
    ...
    case GLFW_KEY_Q:
        ...
        break;
    case GLFW_KEY_F:
        app->orbit.follow = !app->orbit.follow;
        break;
    case GLFW_KEY_P:
        app->orbit.showPoints = !app->orbit.showPoints;
        break;
    case GLFW_KEY_K:
        app->orbit.showKeyframes = !app->orbit.showKeyframes;
        break;
    case GLFW_KEY_G:
        app->orbit.showEdges = !app->orbit.showEdges;
        break;
    case GLFW_KEY_I:
        app->orbit.showInertial = !app->orbit.showInertial;
        break;
    case GLFW_KEY_1:
        SetPresetView(app->orbit, 1);
        break;
    case GLFW_KEY_2:
        SetPresetView(app->orbit, 2);
        break;
    case GLFW_KEY_3:
        SetPresetView(app->orbit, 3);
        break;
    case GLFW_KEY_A:
        AutoFrameOrbit(*app);
        break;
...
void UpdateFrameWindow(AppState& app)
{
    if (!app.hasState || app.viewerState.frameImagePath.empty())
        return;

    const time_t mtime = GetFileModificationTime(app.viewerState.frameImagePath);
    if (mtime == 0)
        return;

    if (app.loadedFrameImagePath == app.viewerState.frameImagePath && app.loadedFrameMTime == mtime)
    {
        cv::waitKey(1);
        return;
    }

    cv::Mat frame = cv::imread(app.viewerState.frameImagePath, cv::IMREAD_COLOR);
    if (frame.empty())
        return;

    app.loadedFrameImagePath = app.viewerState.frameImagePath;
    app.loadedFrameMTime = mtime;
    cv::imshow("ORB-SLAM3: Current Frame", frame);
    cv::waitKey(1);
}
...
void UpdateWindowTitle(GLFWwindow* window, const AppState& app)
{
    ...
    title << "ORB-SLAM3 Native Debug Viewer";
    if (app.hasState)
    {
        size_t referenceCount = 0;
        for (const PointSample& point : app.viewerState.points)
        {
            if (point.isReference)
                ++referenceCount;
        }

        title << " | points " << app.viewerState.points.size()
              << " (" << referenceCount << " ref)"
              << " | keyframes " << app.viewerState.keyframes.size()
              << " | edges " << app.viewerState.edges.size()
              << " | "
              << (app.orbit.showPoints ? "P:on" : "P:off") << " "
              << (app.orbit.showKeyframes ? "K:on" : "K:off") << " "
              << (app.orbit.showEdges ? "G:on" : "G:off") << " "
              << (app.orbit.showInertial ? "I:on" : "I:off");
        if (app.orbit.follow)
            title << " | follow";
    }
    else
...
void RenderScene(const AppState& app, int width, int height)
{
    ...
    if (app.hasState)
    {
        ...
        if (app.orbit.showKeyframes)
            DrawKeyframes(app.viewerState);
        DrawCurrentCamera(app.viewerState);
    }
}
...
{
    AppState app;
    app.snapshotPath = argc > 1 ? argv[1] : "debug_map_state.txt";
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 800, "ORB-SLAM3 Native Debug Viewer", nullptr, nullptr);
    if (!window)
    {
    ...
    cv::namedWindow("ORB-SLAM3: Current Frame", cv::WINDOW_AUTOSIZE);

    while (!glfwWindowShouldClose(window))
    {
        ViewerState nextState;
        if (LoadState(app.snapshotPath, nextState))
        {
            const bool wasEmpty = !app.hasState;
            app.viewerState = nextState;
            app.hasState = true;
            if (wasEmpty && !app.hasAutoFramed)
            {
                AutoFrameOrbit(app);
